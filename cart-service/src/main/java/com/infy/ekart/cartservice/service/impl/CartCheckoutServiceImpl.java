...
public class CartCheckoutServiceImpl implements CartCheckoutService {
    ...
    public CheckoutResponse checkout(String cartId, CheckoutRequest request) {
        ...
        // Get cart with items
        Cart cart = cartRepository.findByIdWithItemsAndPromotions(cartUuid)
        ...
        cartService.markCartAsConverted(cartUuid, order.orderId());

        // Release reservations
        inventoryService.releaseReservations(cartId);

        log.info("Checkout completed for cart: {}, order: {}", cartId, order.orderId());
